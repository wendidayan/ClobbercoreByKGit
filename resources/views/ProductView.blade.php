...
<body style="background: #f5f5f5;">
    ...
    <section id="all-products">
        <div class="new-arrival-heading" style="font-family: 'Open Sans', sans-serif;">
            ...
            <div id="similar-products-section" class="container mt-0 pt-0">
                ...
                @if ($similarProducts->lastPage() > 1)
            <nav class="mt-3">
                <ul class="pagination justify-content-center">
                    {{-- Previous Page Link --}}
                    <li class="page-item {{ $similarProducts->onFirstPage() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $similarProducts->previousPageUrl() }}" aria-label="Previous">
                            <span aria-hidden="true">«</span>
                        </a>
                    </li>
                    ...
                    {{-- Page Numbers --}}
                    @php
                        $start = max(1, $similarProducts->currentPage() - 2);
                        $end = min($similarProducts->lastPage(), $similarProducts->currentPage() + 2);
                    @endphp
                    @for ($i = $start; $i <= $end; $i++)
                        <li class="page-item {{ $i == $similarProducts->currentPage() ? 'active' : '' }}">
                            <a class="page-link" href="{{ $similarProducts->url($i) }}">{{ $i }}</a>
                        </li>
                    @endfor

                    {{-- Next Page Link --}}
                    <li class="page-item {{ !$similarProducts->hasMorePages() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $similarProducts->nextPageUrl() }}" aria-label="Next">
                            <span aria-hidden="true">»</span>
                        </a>
                    </li>
                </ul>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        document.body.addEventListener('click', function (e) {
            const target = e.target.closest('.pagination a');
            if (target && target.closest('#similar-products-section')) {
                e.preventDefault();
                ...
                fetch(target.href)
                    ...
                    .then(html => {
                        const parser = new DOMParser();
                        const doc = parser.parseFromString(html, 'text/html');
                        const newContent = doc.querySelector('#similar-products-section');
                        document.querySelector('#similar-products-section').innerHTML = newContent.innerHTML;
                        history.pushState(null, '', target.href);
                    })
                    .catch(err => console.error('Similar products pagination failed', err));
            }
        });
    });
</script>
